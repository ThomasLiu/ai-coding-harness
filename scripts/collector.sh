...
main() {
    ...
    # 用临时文件代替变量，避免 argument list too long
    TEMP_DIR=$(mktemp -d)
    TEMP_MERGED="$TEMP_DIR/merged.json"
    TEMP_ALL_REPOS="$TEMP_DIR/all_repos.json"
    TEMP_EXISTING="$TEMP_DIR/existing.json"
    TEMP_PROCESSED="$TEMP_DIR/processed.json"
    ...
    for keyword in "${KEYWORDS[@]}"; do
        ...
        if result=$(gh search repos "$keyword" --sort stars --limit 100 --json name,owner,description,url,stargazersCount,createdAt,updatedAt 2>&1); then
            ...
            log "  -> $count 个仓库"
            if $first; then
                echo "$result" > "$TEMP_MERGED"
                first=false
            else
                # 用临时文件合并，避免传参过长
                echo "$result" > "$TEMP_DIR/keyword_result.json"
                jq -s '.[0] + .[1]' "$TEMP_MERGED" "$TEMP_DIR/keyword_result.json" > "$TEMP_MERGED.tmp" 2>/dev/null || true
                if [[ -f "$TEMP_MERGED.tmp" ]]; then
    ...
    # 去重
    log "去重..."
    jq 'unique_by(.url)' "$TEMP_MERGED" > "$TEMP_ALL_REPOS"
    count=$(jq length "$TEMP_ALL_REPOS" 2>/dev/null || echo "0")
    log "去重后: $count 个仓库"
    ...
    # 读取已有仓库数据，构建 URL -> 仓库 索引（写入临时文件避免参数过长）
    if [[ -f "$DATA_FILE" ]]; then
        jq '.repos | INDEX(.url)' "$DATA_FILE" > "$TEMP_EXISTING" 2>/dev/null || echo "{}" > "$TEMP_EXISTING"
    else
        echo "{}" > "$TEMP_EXISTING"
    fi

    # 用 jq 从临时文件读取，避免大 JSON 作为命令行参数
    jq --arg today "$today" --slurpfile existing "$TEMP_EXISTING" '
        map({
            name: .name,
            owner: .owner.login,
            url: .url,
            description: (.description // ""),
            description_zh: ($existing[0][.url].description_zh // ""),
            pros: ($existing[0][.url].pros // []),
            cons: ($existing[0][.url].cons // []),
            analyzed: ($existing[0][.url].analyzed // false),
            stars: (.stargazersCount // 0),
            created_at: .createdAt,
            repo_updated_at: .updatedAt,
            last_collected_at: (now | strftime("%Y-%m-%dT%H:%M:%SZ")),
            last_updated: $today,
            history: (($existing[0][.url].history // []) + [{date: $today, stars: (.stargazersCount // 0)}])
        })
    ' "$TEMP_ALL_REPOS" > "$TEMP_PROCESSED"

    # 更新数据文件
    log "更新数据文件..."
    now_iso=$(date -u +%Y-%m-%dT%H:%M:%SZ)
    echo "{\"version\": \"1.0.0\", \"collected_at\": \"$now_iso\", \"last_collection\": \"$today\", \"repos\": " > "$DATA_FILE"
    cat "$TEMP_PROCESSED" >> "$DATA_FILE"
    echo "}" >> "$DATA_FILE"
